...
@cocktail_router.post("/cocktails/", response_model=Cocktail, status_code=201)
...
async def cocktail_autocomplete(fragment: str):
    ...
    Args:
        fragment (str): text to be searched
    """
    cocktail_list = await Cocktail.aggregate(
        aggregation_pipeline=[
            {
                "$search": {
                    "index": "cocktail_search",
                    "autocomplete": {"query": fragment, "path": "name"},
                }
            }
        ]
    ).to_list()

    return [c["name"] for c in cocktail_list]
